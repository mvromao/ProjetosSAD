...
void setupChannels() {
    ...
    // Set RC1, RC2, RC3 as digital outputs
    TRISCbits.TRISC1 = 0;
    TRISCbits.TRISC2 = 0;
    TRISCbits.TRISC3 = 0;

    // Optionally clear outputs (LEDs OFF initially)
    LATCbits.LATC1 = 0;
    LATCbits.LATC2 = 0;
    LATCbits.LATC3 = 0;
...
void turnOnLED(char led) {
    ...
    switch (led) {
        ...
        case 'z':
            ...
            break;
        default:
            printf("LED não existente\n");
            return;
            break;
    }
     printf("LED acionado: %c,\n",led);
}
...
void I2C2_Init(void) {
    ...
    //I2C2BRG = ((FCY / I2C_BAUDRATE) - FCY / 1111111) - 1; //39 caso não dê
    I2C2BRG = 39;
...
void I2C2_SendToArduino(const char data) {
    I2C2_Start();
    I2C2TRN = SLAVE_ADDR << 1; // Send slave address + Write bit
